...
export function Header() {
  ...
  return (
    <header className="fixed top-0 z-50 w-full">
      <nav className="mx-auto flex items-center justify-between rounded-b-[2rem] magnetik md:px-20 md:py-5">
        ...
        <ul className="hidden gap-8 rounded-[92px] bg-[#F2EFEB1A] backdrop-blur-xl px-8 py-4 text-sm md:flex">
          {links.map((link) => {
        ...
        >
          <span
            aria-hidden
            className={`absolute inset-y-2 left-2 w-[calc(50%-0.5rem)] rounded-full bg-violet shadow-[0_4px_12px_rgb(97_76_222_/_0.35)] transition-transform duration-300 ease-out md:inset-y-3 md:left-3 md:w-[calc(50%-0.75rem)] ${
              language === "en" ? "translate-x-[calc(100%+0.5rem)] md:translate-x-[calc(100%+0.75rem)]" : "translate-x-0"
            }`}
          />
